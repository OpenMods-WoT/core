...
if not hasattr(BigWorld, 'PMC_wasPrint'):
    ...
    print 'Current OpenModsCore version: 1.1.0 (%s)' % time.strftime('%d.%m.%Y', time.localtime(
        os.stat('%s/scripts/client/gui/mods/OpenModsCore.pyc' % BigWorld.curCV).st_mtime))
...
class _Config(object):
    ...
    def do_config(self):
        ...
        try:
            ...
            mod_PYmodsGUI.g_gui.register('%s' % self.ID, self.template_settings, self.data, self.apply_settings,
                                         self.update_settings, self.onApply, self.onWindowClose)
        except ImportError:
